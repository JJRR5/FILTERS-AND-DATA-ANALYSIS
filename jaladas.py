...
def select():
	...
	if filtro>=0 and tipo>=0:
		...
		def evaluar(filtro,tipo):			
			Fs=eval(entryfs.get()/"2")

			if tipo==0:
				btype="lowpass"
				w=eval(entryfc.get()/Fs)
			elif tipo==1:
				btype="highpass"
				w=eval(entryfc.get()/Fs)
			elif tipo==2:
				btype="bandpass"
				w=[eval(entryfc1.get()/Fs,entryfc2.get()/Fs)]
			elif tipo==3:
				btype="bandstop"
				w=[eval(entryfc1.get()/Fs,entryfc2.get()/Fs)]
			
			if filtro==0:
				b,a=signal.butter(entryN.get(),w,btype)
				w,h=signal.freqz(b,a)
				plt.title('Butterworth filter frequency response')
			elif filtro==1:
				b,a=signal.butter(entryN.get(),entryrp.get(),w,btype)
				w,h=signal.freqz(b,a)
				plt.title('Chevyshev I filter frequency response')
			elif filtro==2:
				b,a=signal.butter(entryN.get(),entryrs.get(),w,btype)
				w,h=signal.freqz(b,a)
				plt.title('Chevyshev II filter frequency response')
			elif filtro==3:
				b,a=signal.butter(entryN.get(),entryrp.get(),entryrs.get(),w,btype)
				w,h=signal.freqz(b,a)
